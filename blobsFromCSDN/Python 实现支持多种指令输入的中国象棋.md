					...
					if method1[1] in "三叁七柒3３7７" and method1[2] in "进進退": # 相三进五 等
						...
						if method1[0] in "仕士" and method1[1] in "五伍5５": # 仕士
							...
							if method1[0] != "象":
								...
								print("请按任意键继续。")
								getch()
							p = (2, digit_num_list.index(method1[1]) % 9)
						else:
							return (False, False, "一五九或159纵路上相象仕士指令不匹配，请检查。")
					else:
						return (False, False, "象棋模式下相象仕士指令不匹配，请检查。")
			...
			try:
				...
				if len(tokens) >= 2:
					...
					if 200 == res.status_code and res.text.startswith("move:"):
						...
						return res.text[5:9]
				return AI(3).comput(board, direction, isEcho = False)
			except:
				return AI(3).comput(board, direction, isEcho = False)
...
def play(mode):
	...
	if mode & 0b0010: # 请指定红方走子等级
		print("所有可能的等级如下（等级 5 及更高等级暂未实现将使用等级 4 走子）：")
			...
			if todo in ("M", "m"):
				...
				if len(board.next) == 0:
					...
					if tmp is not None:
						board = tmp # 确认切换
				continue
			elif len(todo) == 1 and todo in "0123456789":
				if board.lists[1] in "WwRr": # 红方走子
					todo = AI(int(todo)).comput(board, True)
				elif board.lists[1] in "BbGg": # 黑方走子
					todo = AI(int(todo)).comput(board, False)
...
![效果图（双方自动下）](https://i-blog.csdnimg.cn/direct/ac6a99a1df4343bd87af278477ac38f0.png)
![效果图（自动判胜负和）](https://i-blog.csdnimg.cn/direct/315f655eacd446ffb069d3a4a1cc10c2.png)



转发自本人 GitHub 项目 [https://github.com/Win-Linux-DOS-Team/chessPokerComputing/tree/main/chChess](https://github.com/Win-Linux-DOS-Team/chessPokerComputing/tree/main/chChess)，目前对棋类游戏和扑克牌游戏的实现和确定性算法感兴趣但没时间研究 ~~（或者知道这些方向发论文比较困难目前要毕不了业了）~~，有兴趣写算法部分的可以 dd ~
